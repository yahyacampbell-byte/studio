...
export default function CognifitGamePage() {
  ...
  const gameLaunchedRef = useRef(false); // To track if CognifitSdk.start has been called for the current game load attempt
  ...
  const initializeAndLoadGame = useCallback(async (accessToken: string | null) => {
    ...
    if (!sdkInitializedRef.current) {
      ...
      try {
        ...
        // Try to get error message from CogniFit's specific error object if available
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      ...
      setCurrentStatus('sdk_error'); // or a more specific 'auth_required' status
      return;
    }
    if (!user) {
    ...
    if (!game && currentStatus !== 'sdk_error' && currentStatus !== 'idle') {
        // Game object not yet set from the other useEffect, but we are past initial idle/error state
        // Potentially set a 'loading_game_details' status or just wait
        return;
    }
    if (!game && currentStatus === 'idle') { // Game object not set, and we are in initial idle state
        return; // Wait for game details to be loaded by the other useEffect
    }
    ...
    const attemptCognifitRegistrationAndLoad = async () => {
      ...
      if (!currentCognifitUserToken) {
        // Ensure all necessary user fields for registration are present
        ...
        try {
          ...
          if (!response.ok || data.error) {
            ...
            console.error("[CognifitGamePage] On-demand Cognitive Gym registration failed (from API route):", errorMessage);
          ...
          if (!newCognifitToken) {
            console.error("[CognifitGamePage] On-demand Cognitive Gym registration failed: No user token received.");
          ...
          console.error("[CognifitGamePage] Error during on-demand Cognitive Gym registration process:", regError);
      ...
      if (currentCognifitUserToken) {
        ...
        try {
          ...
          if (!accessTokenResponse.ok || accessTokenData.error) {
            ...
            console.error("[CognifitGamePage] Failed to fetch access token:", errorMsg);
          ...
          if (!newAccessToken) {
            console.error("[CognifitGamePage] No access token received from API.");
          ...
          // Only call initializeAndLoadGame if SDK isn't already initialized OR if a game has ended (to allow re-launch)
           if (!sdkInitializedRef.current || !gameLaunchedRef.current || currentStatus === 'game_ended') {
             if (currentStatus === 'game_ended') { // Reset flags for re-launch
                sdkInitializedRef.current = false;
                gameLaunchedRef.current = false;
            }
          ...
          console.error("[CognifitGamePage] Error fetching access token:", tokenError);
    ...
    if (game && (!sdkInitializedRef.current || !gameLaunchedRef.current || currentStatus === 'game_ended') ) {
      ...
      initializeAndLoadGame, currentStatus
    ]);


  if (isLoadingAuthContext && !isAuthenticated) { // Show full page loader if still loading auth overall
    return (
      <AppLayout>
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            {showLoadingIndicator && !statusMessage && !showErrorState && ( // Only show generic loader if no specific message
            ...
            {currentStatus === 'game_loaded' && sdkInitializedRef.current && game &&
             typeof document !== 'undefined' && // Ensure document is defined (client-side)
            ...
            {!isAuthenticated && !isLoadingAuthContext && ( // Message if user is not authenticated and auth isn't loading
                 <div className="text-center py-4">
                    <p className="text-muted-foreground">Please log in to play Cognitive Gym games.</p>
                </div>
            )}
             {isAuthenticated && !game && gameKeyParam && currentStatus === 'sdk_error' && ( // If authenticated, game key exists, but game object not found and there's an error
                <div className="text-center py-4 text-muted-foreground">
